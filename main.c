...
/* WolfSSL sockets transport implementation. */
...
static int connectToServerWithBackoffRetries( NetworkContext_t * pNetworkContext )
{
    ...
    WolfsslStatus_t wolfsslStatus = WOLFSSL_SUCCEED;
    RetryUtilsParams_t reconnectParams;
    ServerInfo_t serverInfo;
    WolfsslCredentials_t wolfsslCredentials;
    ...
    wolfsslCredentials.pRootCaPath = Storage_GetAbsolutePathInImagePackage(ROOT_CA_CERT_PATH);
    wolfsslCredentials.pClientCertPath = DeviceAuth_GetCertificatePath();
    wolfsslCredentials.pPrivateKeyPath = NULL;
    ...
    wolfsslCredentials.sniHostName = AWS_IOT_ENDPOINT;
    ...
    if( AWS_MQTT_PORT == 443 )
    {
         ...
         */
        wolfsslCredentials.pAlpnProtos = AWS_IOT_MQTT_ALPN;
        wolfsslCredentials.alpnProtosLen = AWS_IOT_MQTT_ALPN_LENGTH;
    }
    ...
    do
    {
        ...
        LogInfo( ( "Establishing a TLS session to %.*s:%d.",
                   ...
                   AWS_IOT_ENDPOINT,
                   AWS_MQTT_PORT ) );
        wolfsslStatus = Wolfssl_Connect( pNetworkContext,
                                         &serverInfo,
                                         &wolfsslCredentials,
                                         TRANSPORT_SEND_RECV_TIMEOUT_MS,
                                         TRANSPORT_SEND_RECV_TIMEOUT_MS );

        if( wolfsslStatus != WOLFSSL_SUCCEED )
        {
    ...
    } while( ( wolfsslStatus != WOLFSSL_SUCCEED ) && ( retryUtilsStatus == RetryUtilsSuccess ) );
                           ...
                           NetworkContext_t * pNetworkContext )
{
     ...
     * from network. Network context is SSL context for WolfSSL.*/
    transport.pNetworkContext = pNetworkContext;
    transport.send = Wolfssl_Send;
    transport.recv = Wolfssl_Recv;
...
static bool isNetworkInterfaceConnectedToInternet(void)
{
    ...
    if (Networking_GetInterfaceConnectionStatus(networkInterface, &status) != 0) {
        if (errno != EAGAIN) {
            LogError( ( "Networking_GetInterfaceConnectionStatus: %d (%s).", errno, strerror( errno ) ) );
            return false;
        }
        LogWarn( ( "Not doing download because the networking stack isn't ready yet." ) );
        return false;
    }

    if ((status & Networking_InterfaceConnectionStatus_ConnectedToInternet) == 0) {
        LogInfo( ( "No internet connectivity." ) );
        return false;
    }
...
int main(void)
{
    ...
    do {
        (void)Application_IsDeviceAuthReady(&isDaaPassed);
        if (!isDaaPassed) {
            LogInfo( ( "DAA is not ready." ) );
        }
        Clock_SleepMs( 1000 );

    } while (isDaaPassed == false);
    ...
    if( returnStatus == EXIT_SUCCESS )
    {
        for( ; ; )
        {
            ...
            ( void ) Wolfssl_Disconnect( &networkContext );
